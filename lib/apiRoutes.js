...
function registerApiRoutes(app, deps) {
  const {
    ...
    updateUserPassword, updateUserGender,
  ...
  app.put("/api/profile", authMiddleware, async (req, res) => {
    try {
      ...
      if (typeof req.body.onboarding_completed === "boolean") {
        patch.onboarding_completed = req.body.onboarding_completed;
      }
      if (req.body.color_theme === null || req.body.color_theme === "") {
        patch.color_theme = null;
      } else if (["pink", "blue", "dark"].includes(req.body.color_theme)) {
        patch.color_theme = req.body.color_theme;
      }
  ...
  app.patch("/api/users/:id/password", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      ...
      await logActivity(req.user.id, "password_reset", { targetId: id });
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.patch("/api/users/:id/gender", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      const id = parseInt(req.params.id, 10);
      const gender = req.body.gender === null || req.body.gender === "" ? null : req.body.gender;
      if (gender !== null && gender !== "male" && gender !== "female") {
        return res.status(400).json({ error: "Gender must be male, female, or null" });
      }
      const target = await findUserById(id);
      if (!target) return res.status(404).json({ error: "User not found" });
      const oldGender = target.gender || null;
      if (oldGender === gender) {
        return res.json({ ok: true, user: { id: target.id, username: target.username, role: target.role, gender } });
      }
      await updateUserGender(id, gender);
      await logActivity(req.user.id, "gender_changed", {
        targetId: id,
        targetUsername: target.username,
        from: oldGender,
        to: gender,
      });
      res.json({ ok: true, user: { id: target.id, username: target.username, role: target.role, gender } });
    } catch (err) {
      res.status(500).json({ error: err.message, hint: err.hint });
    }
  });
