...
async function updateProfile(userId, patch) {
  if (useSupabase()) {
    let payload = { user_id: userId, ...patch, updated_at: new Date().toISOString() };
    let { error } = await sb().from("user_profiles").upsert(payload);
    if (error && String(error.message || error).includes("color_theme")) {
      const { color_theme, ...rest } = payload;
      ({ error } = await sb().from("user_profiles").upsert(rest));
    }
...
async function disableUser(id) {
  if (useSupabase()) {
    const { ensureUserColumns } = require("./db-supabase");
