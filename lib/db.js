...
async function findUserByUsername(username) {
  if (getSupabaseDb()) return getSupabaseDb().findUserByUsername(username);
...
async function findUserById(id) {
  if (getSupabaseDb()) return getSupabaseDb().findUserById(id);
  const db = await readDb();
...
async function getAllUsers() {
  if (getSupabaseDb()) return getSupabaseDb().getAllUsers();
  const db = await readDb();
  return db.users
    .map(({ id, username, role, created_at, disabled_at }) => ({
      id, username, role, created_at, disabled_at: disabled_at || null,
    }))
    .sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
}

async function createUser(username, passwordHash, role) {
  if (getSupabaseDb()) return getSupabaseDb().createUser(username, passwordHash, role);
  const db = await readDb();
  const user = {
    id: db.nextId++,
    username,
...
async function deleteUser(id) {
  if (getSupabaseDb()) return getSupabaseDb().deleteUser(id);
  const db = await readDb();
...
async function updateUserPassword(id, passwordHash) {
  if (getSupabaseDb()) return getSupabaseDb().updateUserPassword(id, passwordHash);
  const db = await readDb();
  const user = db.users.find((u) => u.id === id);
...
async function getUserPlans(userId) {
  if (getSupabaseDb()) return getSupabaseDb().getUserPlans(userId);
  const db = await readDb();
...
async function getUserPlanById(userId, planId) {
  if (getSupabaseDb()) return getSupabaseDb().getUserPlanById(userId, planId);
  const plans = await getUserPlans(userId);
  return plans.find((p) => p.id === planId) || null;
}

async function addUserPlan(userId, plan) {
  if (getSupabaseDb()) return getSupabaseDb().addUserPlan(userId, plan);
  const db = await readDb();
  const key = String(userId);
...
async function updateUserPlan(userId, planId, updates) {
  if (getSupabaseDb()) return getSupabaseDb().updateUserPlan(userId, planId, updates);
  const db = await readDb();
  const key = String(userId);
  const plans = normalizeUserPlans(db.plans[key]);
...
async function deleteUserPlanById(userId, planId) {
  if (getSupabaseDb()) return getSupabaseDb().deleteUserPlanById(userId, planId);
  const db = await readDb();
  const key = String(userId);
