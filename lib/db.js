...
function isPersistentStorage() {
  ...
  return mode === "supabase" || mode === "redis" || mode === "blob" || mode === "file";
}
...
async function withSupabase(fn, localFn) {
  if (!getSupabaseDb()) return localFn();
  // Never fall back when Supabase is configured — falling back to Vercel /tmp
  // makes create-user "succeed" then vanish on the next cold start.
  return fn();
}

async function findUserByUsername(username) {
  return withSupabase(
    () => getSupabaseDb().findUserByUsername(username),
    async () => {
      const db = await readDb();
      const lower = username.toLowerCase();
      const user = db.users.find((u) => u.username.toLowerCase() === lower) || null;
...
async function getAllUsers() {
  return withSupabase(
    () => getSupabaseDb().getAllUsers(),
    async () => {
      const db = await readDb();
      return db.users
        .map(({ id, username, role, gender, created_at, disabled_at }) => ({
          id, username, role, gender: gender || null, created_at, disabled_at: disabled_at || null,
        }))
...
async function createUser(username, passwordHash, role) {
  return withSupabase(
    () => getSupabaseDb().createUser(username, passwordHash, role),
    async () => {
      const db = await readDb();
      const user = {
        id: db.nextId++,
...
async function updateUserGender(id, gender) {
  return withSupabase(
    () => getSupabaseDb().updateUserGender(id, gender),
    async () => {
      const db = await readDb();
      const user = db.users.find((u) => u.id === id);
...
async function updateUserPassword(id, passwordHash) {
  return withSupabase(
    () => getSupabaseDb().updateUserPassword(id, passwordHash),
    async () => {
      const db = await readDb();
      const user = db.users.find((u) => u.id === id);
      if (!user) throw new Error("User not found");
...
async function getUserPlans(userId) {
  return withSupabase(
    () => getSupabaseDb().getUserPlans(userId),
    async () => {
      const db = await readDb();
...
async function getUserPlanById(userId, planId) {
  return withSupabase(
    () => getSupabaseDb().getUserPlanById(userId, planId),
    async () => {
...
async function addUserPlan(userId, plan) {
  return withSupabase(
    () => getSupabaseDb().addUserPlan(userId, plan),
    async () => {
      const db = await readDb();
      const key = String(userId);
...
async function updateUserPlan(userId, planId, updates) {
  return withSupabase(
    () => getSupabaseDb().updateUserPlan(userId, planId, updates),
    async () => {
      const db = await readDb();
      const key = String(userId);
      const plans = normalizeUserPlans(db.plans[key]);
...
async function deleteUserPlanById(userId, planId) {
  return withSupabase(
    () => getSupabaseDb().deleteUserPlanById(userId, planId),
    async () => {
      const db = await readDb();
      const key = String(userId);
