...
const supabaseDb = useSupabase() ? require("./db-supabase") : null;
...
async function findUserByUsername(username) {
  if (supabaseDb) return supabaseDb.findUserByUsername(username);
...
async function findUserById(id) {
  if (supabaseDb) return supabaseDb.findUserById(id);
  const db = await readDb();
...
async function getAllUsers() {
  if (supabaseDb) return supabaseDb.getAllUsers();
  const db = await readDb();
  return db.users
...
async function createUser(username, passwordHash, role) {
  if (supabaseDb) return supabaseDb.createUser(username, passwordHash, role);
  const db = await readDb();
  const user = {
    id: db.nextId++,
    username,
...
async function deleteUser(id) {
  if (supabaseDb) return supabaseDb.deleteUser(id);
  const db = await readDb();
...
async function getUserPlans(userId) {
  if (supabaseDb) return supabaseDb.getUserPlans(userId);
  const db = await readDb();
...
async function getUserPlanById(userId, planId) {
  if (supabaseDb) return supabaseDb.getUserPlanById(userId, planId);
  const plans = await getUserPlans(userId);
  return plans.find((p) => p.id === planId) || null;
}

async function addUserPlan(userId, plan) {
  if (supabaseDb) return supabaseDb.addUserPlan(userId, plan);
  const db = await readDb();
  const key = String(userId);
...
async function updateUserPlan(userId, planId, updates) {
  if (supabaseDb) return supabaseDb.updateUserPlan(userId, planId, updates);
  const db = await readDb();
  const key = String(userId);
  const plans = normalizeUserPlans(db.plans[key]);
...
async function deleteUserPlanById(userId, planId) {
  if (supabaseDb) return supabaseDb.deleteUserPlanById(userId, planId);
  const db = await readDb();
  const key = String(userId);
